...
from include.transform.queries import read_silver_data_into_table, total_products_sold_by_category

class loadProductsCountToGold(BaseOperator): 
    ...
    def execute(self, context):
        ...
        self.log.info('Aggregating products by category')
        self.conn.sql(total_products_sold_by_category)

        self.log.info('writing products aggregation to gold layer')
        try:
            self.conn.sql(write_delta_to_s3(self.table_name, self.conn, "gold"))
            self.log.info('Products aggregation is written successfully to gold layer!')
